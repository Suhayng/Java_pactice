...
/*
  	 여러 쓰레드가 공유된 필드에 접근 할 때 동시성의 문제가 발생 ==> 동기화는 동시성 문제를 해결
 
  	 동기화 : 여러 쓰레드가 공유된 필드를 접근할 때 하나의 쓰레드가 완료되면 다른 쓰레드에서 접근 가능
  		     하나의 쓰레드가 사용중인 경우 lock 을 걸고 완료되면 다른 쓰레드가 접근 가능하도록 Lock 해제

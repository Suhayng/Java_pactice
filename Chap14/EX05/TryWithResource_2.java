...
// 직접 생성한 객체에서 AutoCloseable 을 사용해보기, 
	// try ( 객체 생성 구문 ) {} catch {} : 자동으로 close() 된다. // try (with Resource)
	...
	// 예외를 처리하는 방법 
		...
		// 2. throws 를 통해서 예외를 처리 하는 방법	  : 예외를 내가 처리하지 않고 호출하는 곳에서 예외를 처리 하도록 미룬다. 
				// 예외 전가. 메소드 예외 전가
				// *throw  :  예외를 강제로 발생시킨다. 
...
class A implements AutoCloseable{ 
	...
	A (String resource) {	// 생성자의 매개 변수로 String 값을 받아서 메모리로 로딩 
...
public class TryWithResource_2 {
	public static void main(String[] args) {
		...
		}	// fianlly 블락에 a1.close() 메소드를 자동으로 호출됨.

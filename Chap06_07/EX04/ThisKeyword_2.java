...
class Aa {				// this key를 사용하지 않는 경우 : 지역 변수로 인식
	int m;
	int n;
...
class Bb {				// this key를 사용하는 경우
	...
	int n ;
	void init (int m, int n) {
		this.m = m;		// this 키를 명시적으로 반드시 사용해서 m 이 필드의 m 이라고 알려줘야 한다.
		this.n = n;
	}
...
public class ThisKeyword_2 {
	public static void main(String[] args) {
		...
		// 1. 필드의 명과 지역 변수의 명이 같은 경우 ( this 키워드를 사용하지 않는 경우 : 지역 변수에 저장 )
		 Aa aa = new Aa();
		 aa.init(2,3);		
		 System.out.println(aa.m);		//0
		 System.out.println(aa.n);		//0
		
		// 2. 필드의 명과 지역 변수의 명이 같은 경우 ( this 키워드를 사용하는 경우 : 필드 변수에 저장 )
		Bb bb = new Bb();
		bb.init(2,3);

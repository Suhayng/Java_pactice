...
// 기본 생성자 : class A 는 기본 생성자가 생략 되어 있다.
// 일반적으로 클래스 블락에 1. 필드 2. 생성자 3. 메소드 형식으로 작성한다.
// 생성자의 역할 : 1. 객체 생성, 2. 필드 값을 초기화

class A {
	int m;			// 필드(클래스 블락에 선언된 변수) : 	데이터 타입 변수명 
						// 힙 영역에 저장, 객체화 시켜야 사용 가능, 예외) static,
						// 힙 영역의 RAM 에는 반드시 초기값이 들어가야 한다.
	
//	A () {}			// <== 기본 생성자	: 클래스 이름과 동일하면서 리턴 타입이 없는 메소드를 생성자라 호칭한다.
					// 기본 생성자는 생략 가능하다.
					// 기본 생성자는 매개 변수도 없고, 실행부도 가지지 않는 생성자.
	
	void work () {	// 메소드 : 		리턴 타입 메소드명 (인풋 매개 변수) { 실행부 }
						// 메소드 호출 : work() 
...
class C {
	...
	C(int c) {					// 생성자의 인풋값을 받아서 메모리의 기본값으로 m 변수 값을 로드 시킨다.
		m = c ;
	}	
	
	void work ( ) {
		System.out.println(m);	// m 은 객체 생성시 생성자에 정수 값을 넣어서 필드 m을 초기화 시켜줌
...
public class DefaultConstructor {
	...
	public static void main(String[] args) {
		// 클래스 내에 포함될 수 있는 것 : 1. 필드  2. 메소드  3. 생성자  4. 인어 클래스(innerClass)	
		// 생성자의 2 가지 특징 : 1. 클래스 이름과 동일한 이름을 가진 메소드를 생성자라 한다. 
		...
		// 기본 생성자 ( Default Constructor) : 
			// 인풋 매개 변수가 없는 생성자, 실행부의 값도 비어 있는 상태의 생성자.
			// 기본 생성자는 생략 가능하다.
	...
	//		C cccc new C();			// 오류 발생 : 기본 생성자가 생략, 다른 생성자가 존재 <<기본 생성자 호출>>
	
			D d = new D ();			// 기본 생성자 호출, 객체의 필드의 초기값 (m:0, n:0, l:0)
			d.work();
	
			D dd = new D(3,4,5);	// 매개 변수가 정수 3개인 생성자 호출
			dd.work();
			
			D ddd = new D(1,2);		// 매개 변수가 정수 3개인 생성자를 호출
			// 생성자 : 메소드 이름이 객체 이름과 리턴 값이 없는 메소드
			// 객체 내의 필드의 값을 초기화 할 때 오버로딩 (Overloading) 에 의해서 해당 생성자가 호출
				// 오버로딩 : 메소드명(생성자명, 데이터 타입, 매개 변수 개수) <== 시그니쳐
				//
